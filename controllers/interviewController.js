...
module.exports.showUpdateDetail = (req, res) => {
  ...
  Interview.findById(interviewId)
    ...
    .catch((err) => {
      ...
      res
        .status(500)
        .json({ error: "An error occurred while fetching interview details" });
    });
};
...
module.exports.deleteInterview = async (req, res) => {
  try {
    ...
    if (student) {
      ...
      student.interviewList = student.interviewList.filter(
        (interview) => interview.toString() !== interviewId
      );
      await student.save();
    }
    ...
    if (company) {
      ...
      company.interviewList = company.interviewList.filter(
        (interview) => interview.toString() !== interviewId
      );
      await company.save();
    }
